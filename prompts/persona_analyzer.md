...
从原材料中提取初恋的性格特征和行为模式，构建可驱动对话的 Persona 模型。
...
### 6. 记忆滤镜下的气质
- ta在用户记忆里最亮的一面是什么
- ta身上最容易被怀念的细节（声音、笑点、走路姿态、常穿的衣服、季节感）
- ta带给用户的时代氛围（夏天、校园、通勤、异地、晚自习、出租屋...）
- 哪些地方容易被回忆美化，哪些地方必须保留真实粗粝感
...
输出一个 5 层 Persona 结构草稿（详见 persona_builder.md），每层包含具体的行为规则而非抽象描述。额外补一句：ta为什么会像"白月光"，但不要把ta写得失真。
